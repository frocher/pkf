...
2. **Collect every Decision with `decision_status: Proposed`** in scope.
...
4. **Ask which to rule on.** For each:
   - **Approve** — propose setting `decision_status: Approved`,
     `decision_date`, and a `rationale` capturing why (draft one from
     the conversation if the director states it in passing; ask for
     it if not).
   - **Decline** — Decision's `decision_status` enum is closed (see
     SKILL.md's object map) and has no "declined" value, so a decline
     is not a status value on its own. Record the ruling in the body's
     ...
     `decision_status: Superseded` and `superseded_by` to the new one's id —
